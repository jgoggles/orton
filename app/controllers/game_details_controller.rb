class GameDetailsController < ApplicationController
  ...
  def index
    @game_details = Year.find_by_name(2011).game_details.order("date ASC")
  ...
  def update
    ...
    respond_to do |format|
      if @game_detail.update_attributes(params[:game_detail])
        ...
        format.html { redirect_to pick_center_path(@game_detail.game.week.year.name, @game_detail.game.week.name), notice: 'Game detail was successfully updated.' }
        format.json { head :ok }
      else
